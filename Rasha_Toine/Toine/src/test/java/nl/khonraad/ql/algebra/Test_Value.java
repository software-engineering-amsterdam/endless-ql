...
public class Test_Value {
    ...
    public void not_boolean() {
        qt().forAll( booleans().all() ).check( (
                φ ) -> guarded_apply( new Value( φ ), "!" ).equals( new Value( !φ ) ) );
    }
    ...
    public void boolean_and_boolean() {
        ...
        qt().forAll( booleans().all(), booleans().all() ).check( ( φ,
                ψ ) -> guarded_apply( new Value( φ ), "&&", new Value( ψ ) ).equals( new Value( φ && ψ ) ) );
    }
    ...
    public void boolean_compared_with_boolean() {
        qt().forAll( booleans().all(), booleans().all() ).check( ( φ,
                ψ ) -> guarded_apply( new Value( φ ), "==", new Value( ψ ) ).equals( new Value( φ.equals( ψ ) ) ) );
    }
    ...
    public void string_plus_string() {
        qt().forAll( strings().basicLatinAlphabet().ofLengthBetween( 0, MAX_STRINGLENGTH_TO_TEST ), strings().basicLatinAlphabet().ofLengthBetween( 0, MAX_STRINGLENGTH_TO_TEST ) ).check( (
                left_string,
                right_string ) -> guarded_apply( new Value( left_string ), "+", new Value( right_string ) ).equals( new Value( left_string
                        + right_string ) ) );
    }
    ...
    public void string_plus_integer() {
        qt().forAll( strings().basicLatinAlphabet().ofLengthBetween( 0, MAX_STRINGLENGTH_TO_TEST ), integers().all() ).check( (
                string,
                integer ) -> guarded_apply( new Value( string ), "+", new Value( integer ) ).equals( new Value( string
                        + integer ) ) );
    }
    ...
    public void string_plus_money() {
        qt().forAll( strings().basicLatinAlphabet().ofLengthBetween( 0, MAX_STRINGLENGTH_TO_TEST ), randomBigDecimals() ).check( (
                string,
                money ) -> guarded_apply( new Value( string ), "+", new Value( money ) ).equals( new Value( string
                        + money.toString() ) ) );
    }
    ...
    public void min_integer() {
        qt().forAll( integers().all() ).check( (
                left_integer ) -> guarded_apply( new Value( left_integer ), "-" ).equals( new Value( -left_integer ) ) );
    }

    @Test
    public void integer_times_integer() {
        qt().forAll( integers().all(), integers().all() ).check( ( left_integer,
                right_integer ) -> guarded_apply( new Value( left_integer ), "*", new Value( right_integer ) ).equals( new Value( left_integer
                        * right_integer ) ) );
    }
    ...
    public void integer_compared_with_integer() {
        qt().forAll( integers().all(), integers().all() ).check( ( left_integer,
                right_integer ) -> guarded_apply( new Value( left_integer ), "==", new Value( right_integer ) ).equals( new Value( left_integer.equals( right_integer ) ) ) );
    }

    @Test
    public void integer_isLowerThen_integer() {
        qt().forAll( integers().all(), integers().all() ).check( ( left_integer,
                right_integer ) -> guarded_apply( new Value( left_integer ), "<", new Value( right_integer ) ).equals( new Value( left_integer < right_integer ) ) );
    }
    ...
    public void money_times_integer() {
        qt().forAll( randomBigDecimals(), integers().all() ).check( ( left_money,
                right_integer ) -> guarded_apply( new Value( left_money ), "*", new Value( right_integer ) ).equals( new Value( left_money.multiply( new BigDecimal( right_integer ) ) ) ) );
    }

    @Test
    public void money_plus_money() {
        qt().forAll( randomBigDecimals(), randomBigDecimals() ).check( ( left,
                right ) -> guarded_apply( new Value( left ), "+", new Value( right ) ).equals( new Value( left.add( right ) ) ) );
    }
    ...
    public void date_plus_integer() {
        qt().forAll( randomDate(), integers().from( 0 ).upTo( 365 * 1000 ) ).check( ( date,
                right_integer ) -> guarded_apply( new Value( new DateTime( date ) ), "+", new Value( right_integer ) ).equals( new Value( new DateTime( date ).plusDays( right_integer ) ) ) );
    }

    @Test
    public void date_min_integer() {
        qt().forAll( randomDate(), integers().from( 0 ).upTo( 365 * 1000 ) ).check( ( date,
                right_integer ) -> guarded_apply( new Value( new DateTime( date ) ), "-", new Value( right_integer ) ).equals( new Value( new DateTime( date ).minusDays( right_integer ) ) ) );
    }
