...
object Style {
  ...
  def apply(widgetType: Option[nl.uva.se.sc.niro.qls.model.ast.style.WidgetType]): WidgetType =
    widgetType.map(Style(_)).getOrElse(DefaultWidget())

  def apply(widgetType: nl.uva.se.sc.niro.qls.model.ast.style.WidgetType): WidgetType = widgetType match {
    case nl.uva.se.sc.niro.qls.model.ast.style.SpinBox(minimum, maximum, stepSize) =>
      SpinBox(minimum, maximum, stepSize)
