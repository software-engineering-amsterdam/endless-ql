...
if [ "$#" -ne "3" ]; then
  ...
  cat << EOF
  USAGE
    $ ./antlr-tester.sh GRAMMAR INPUT RULE
  PARAMETERS
...
{
  if [ ! -f "$1" ]; then
    echo "No ANTLR JAR found, downloading and remove afterwards"
...
# Teardown, delete all working files
rm *.java
rm *.class
rm *.tokens
rm *.jar
