...
namespace QuestionnaireDomain.Entities.Domain
{
    internal class SymbolTable : ISymbolTable
    {
        ...
        {
            if (!Exists(variableRef)) throw new ArgumentException(@"attempt to access a non existant variable");
        ...
        public void Add<T>(Guid variableRef, T value)
        {
            ...
            if (m_symbols.ContainsKey(variableRef)) return;
        ...
        {
            if (newDataType == originalDataType) return value;

            if (newDataType.IsSubclassOf(originalDataType)) return value;
            ...
            if (originalDataType == typeof(int)) return ParseInt(stringValue);

            if (originalDataType == typeof(DateTime)) return ParseDate(stringValue);

            if (originalDataType == typeof(decimal)) return ParseDecimal(stringValue);

            if (originalDataType == typeof(bool)) return ParsedBool(stringValue);
        ...
        {
            bool parsedBool;
            if (bool.TryParse(stringValue, out parsedBool)) return parsedBool;
        ...
        {
            decimal parsedNum;
            if (decimal.TryParse(stringValue, out parsedNum)) return parsedNum;
        ...
        private static DateTime ParseDate(string stringValue)
        {
            ...
            throw new ArgumentException(
                $"in the symbolTable there was an attempt to convert {stringValue} to DateTime");
        }

        private static int ParseInt(string stringValue)
        {
            int parsedInt;
            if (int.TryParse(stringValue, out parsedInt)) return parsedInt;
