...
public class IdentifierResolverTests {
	...
	public void undeclaredQuestionName1() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(undeclaredQuestionName1);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 2, column: 26: Undeclared identifier: q2"));
	}
	...
	public void undeclaredQuestionName2() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(undefinedQuestionName2);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
	...
	public void readOnlyQuestionName1() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(readOnlyQuestionName1);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 4, column: 12: Read-only identifier is already declared in an outside scope (line: 2, column: 12): q1"));
	}
	...
	public void readOnlyQuestionName2() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(readOnlyQuestionName2);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
	...
	public void readOnlyQuestionName3() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(readOnlyQuestionName3);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
	...
	public void readOnlyQuestionName4() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(readOnlyQuestionName4);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 4, column: 12: Read-only identifier already declared the current scope: q1"));
	...
	public void readOnlyQuestionName5() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(readOnlyQuestionName5);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 4, column: 12: Read-only identifier is already declared in an outside scope (line: 2, column: 12): q1"));
	...
	public void cyclicDependencyTest() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(cyclicDependencyTest);
		identifierResolver.resolve(ast);
	...
	public void sameNameDifferentTypeTest1() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(sameNameDifferentTypeTest1);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 3, column: 12: Read-only identifier already declared the current scope: q1"));
	...
	public void sameNameDifferentTypeTest2() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(sameNameDifferentTypeTest2);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 4, column: 12: Read-only identifier with a different type is declared in an outside scope (line: 2, column: 12): q1"));
	}
	...
	public void positiveTest1() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(positiveTest1);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 0);
	}

	String  sameNameTest1 =
			"form Form1 {\n"
			+ "\"question1\" q1: boolean\n"
			+ "if (q1) {\n"
			+ "\"question1\" q1: boolean\n"
	...
	@Test
	public void sameNameTest1() throws Exception {
		List<Statement> ast = TestUtilities.buildAst(sameNameTest1);
		identifierResolver.resolve(ast);
		assertTrue(identifierResolver.getNumberOfErrors() == 1);
		assertTrue(identifierResolver.
				getErrorAtIndex(0).equals("[IdentifierResolver] line: 4, column: 12: Read-only identifier is already declared in an outside scope (line: 2, column: 12): q1"));
